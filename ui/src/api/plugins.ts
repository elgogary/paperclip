...
// ─── Toolkit Plugin API (company-scoped plugin management) ───────────────────
...
export const toolkitPluginsApi = {
  ...
  remove: (companyId: string, pluginId: string) =>
    api.delete<{ ok: boolean }>(`/companies/${companyId}/plugins/${pluginId}`),
  test: (companyId: string, pluginId: string) =>
    api.post<{ ok: boolean; latencyMs?: number }>(
  ...
  updateAccess: (companyId: string, pluginId: string, access: { agentId: string; granted: boolean }[]) =>
    api.put<{ ok: boolean }>(`/companies/${companyId}/plugins/${pluginId}/access`, { access }),
};
