...
export function issueCommentRoutes(services: IssueRouteServices) {
  const router = Router();
  const { svc, heartbeat, storage } = services;
  ...
  router.post("/issues/:id/comments", validate(addIssueCommentSchema), async (req, res) => {
    ...
    assertCompanyAccess(req, issue.companyId);
    if (!(await assertAgentRunCheckoutOwnership(services, req, res, issue))) return;
