...
export function sanadBrainRoutes(_db: Db) {
  ...
  router.all("/brain/*path", async (req, res) => {
    ...
    try {
      ...
      if (isMultipart) {
        // Pipe multipart body directly — don't set Content-Type (let fetch derive it)
        // Actually express.json() already consumed the stream so we need a workaround
        // For now, return error — uploads go direct to Sanad Brain
        res.status(501).json({ error: "File uploads not yet supported via proxy. Use direct API." });
        return;
      }
