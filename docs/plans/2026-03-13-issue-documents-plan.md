...
Add first-class **documents** to Sanad AI EOI as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Sanad AI EOI workflow/docs.
...
Update the Sanad AI EOI agent workflow so planning no longer edits the issue description.
