...
Audience: Operators setting up and running agents in Sanad AI EOI

## 1. What this system does

Agents in Sanad AI EOI do not run continuously.  
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`), Sanad AI EOI assumes the CLI is already installed and authenticated on the host machine.
...
Sanad AI EOI stores session IDs for resumable adapters.
...
Sanad AI EOI pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Sanad AI EOI surfaces this as a warning in environment tests, not a hard error.
