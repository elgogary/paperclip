...
How to get OpenClaw running in a Docker container for local development and testing the Sanad AI EOI OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Sanad AI EOI includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-sanad-ai-eoi-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Sanad AI EOI host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-sanad-ai-eoi-smoke`)
...
If your Sanad AI EOI deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Sanad AI EOI server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Sanad AI EOI URL (typically `http://host.docker.internal:3100`).
- If Sanad AI EOI rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm sanadai allowed-hostname host.docker.internal
```

Then restart Sanad AI EOI and rerun the smoke script.
...
```bash
pnpm sanadai allowed-hostname <host>
```
