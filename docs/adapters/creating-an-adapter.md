...
Build a custom adapter to connect Sanad AI EOI to any agent runtime.
...
`format-event.ts` — pretty-prints stdout for `sanadai run --watch` using `picocolors`.
...
Make Sanad AI EOI skills discoverable to your agent runtime without writing to the agent's working directory:
