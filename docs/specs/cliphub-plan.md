# ClipHub: Marketplace for Sanad AI EOI Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Sanad AI EOI configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Sanad AI EOI-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Sanad AI EOI company (agents, projects, governance) |
...
A complete Sanad AI EOI company configuration:
...
Single-agent configurations designed to plug into a Sanad AI EOI org:
...
Portable skill files that any Sanad AI EOI agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Sanad AI EOI's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Sanad AI EOI version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Sanad AI EOI company
...
| `POST` | `/api/listings/:id/install` | Install to Sanad AI EOI company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Sanad AI EOI company (or create new)
  → ClipHub API calls Sanad AI EOI API to:
  ...
  → Redirect to Sanad AI EOI dashboard with new team running
```
...
### 5.3 Creator: Export from Sanad AI EOI → Publish

```
Running Sanad AI EOI company → "Export as Blueprint" (CLI or UI)
  → Sanad AI EOI exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Sanad AI EOI brand blue/purple)
...
| Install flow | One-click deploy to Sanad AI EOI company |
...
{
  "targetCompanyId": "uuid",         // Existing Sanad AI EOI company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Sanad AI EOI
- **Backend**: Node.js API (or extend Sanad AI EOI server)
- **Database**: Postgres (can share Sanad AI EOI's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Sanad AI EOI auth (or OAuth2)

### 9.2 Integration with Sanad AI EOI

ClipHub can be:
- **Option A**: A separate app that calls Sanad AI EOI's API to install blueprints
- **Option B**: A built-in section of the Sanad AI EOI UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Sanad AI EOI UI and API.
...
- [ ] Install flow: blueprint → Sanad AI EOI company
...
- [ ] "Export from Sanad AI EOI" CLI command
