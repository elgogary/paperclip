...
Sanad AI EOI supports two runtime modes with different security profiles.
...
When migrating from `local_trusted` to `authenticated`, Sanad AI EOI emits a one-time claim URL at startup:
